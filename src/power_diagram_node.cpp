...
public:
    ...
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;  // Need this since we have SO3Control which
...
private:
    ...
    void
    odom_callback(const nav_msgs::Odometry::ConstPtr &odom);
    ...
    std::string position_cmd_frame_id_;
...
void
PowerDiagramControlNode::publishDDCommand(void) {
    if (!have_odom_) {
...
void
PowerDiagramControlNode::position_cmd_callback(const geometry_msgs::PoseStamped::ConstPtr &cmd) {
    position_cmd_frame_id_ = cmd->header.frame_id;

    des_pos_ = Eigen::Vector3d(cmd->pose.position.x, cmd->pose.position.y, cmd->pose.position.z);
    // des_vel_ = Eigen::Vector3f(cmd->velocity.x, cmd->velocity.y,
    // cmd->velocity.z);
...
void
PowerDiagramControlNode::odom_callback(const nav_msgs::Odometry::ConstPtr &odom) {
    have_odom_ = true;
    odom_frame_id_ = odom->header.frame_id;
    if (!position_cmd_frame_id_.empty() && odom_frame_id_ != position_cmd_frame_id_) {
        ROS_WARN_ONCE(
            "Odom frame %s does not match position command frame %s",
            odom_frame_id_.c_str(),
            position_cmd_frame_id_.c_str());
    }
    ...
    if (position_cmd_active_) {
        ...
        if (!position_cmd_updated_) publishDDCommand();
        position_cmd_updated_ = false;
...
void
PowerDiagramControlNode::pose_callback(const geometry_msgs::PoseStamped::ConstPtr &pose) {
    have_odom_ = true;
    odom_frame_id_ = pose->header.frame_id;
    if (!position_cmd_frame_id_.empty() && odom_frame_id_ != position_cmd_frame_id_) {
        ROS_WARN_ONCE(
            "Pose frame %s does not match position command frame %s",
            odom_frame_id_.c_str(),
            position_cmd_frame_id_.c_str());
    }
...
void
PowerDiagramControlNode::onInit(void) {
    ros::NodeHandle n("~");
    ...
    odom_sub_ = n.subscribe(
        "odom",
        10,
        &PowerDiagramControlNode::odom_callback,
        this,
        ros::TransportHints().tcpNoDelay());
    pose_sub_ = n.subscribe(
        "pose",
        10,
        &PowerDiagramControlNode::pose_callback,
        this,
        ros::TransportHints().tcpNoDelay());
    position_cmd_sub_ = n.subscribe(
        "position_cmd",
        10,
        &PowerDiagramControlNode::position_cmd_callback,
        this,
        ros::TransportHints().tcpNoDelay());
    // Setup Publisher
    ...
    ROS_INFO(
        "PowerDiagramControlNode is initialized with gains/ka: %f, gains/kl: %f, "
        "vmax: %f, wmax: %f",
        ka,
        kl,
        vmax,
