...
void
PowerDiagramControl::calculateControl(
    double des_x,
    double des_y,
    double ka,
    double kl,
...
bool
PowerDiagramControl::checkReachedGoal(const Eigen::Vector3d &des_pos, double des_yaw) {
    bool result = false;
    double dx = pos_[0] - des_pos[0];
    double dy = pos_[1] - des_pos[1];
    if (std::hypot(dx, dy) < 0.3) result = true;
    return result;
}
