...
void
DDControl::setPosition(const Eigen::Vector3d &position) {
    pos_ = position;
}

void
DDControl::setVelocity(const Eigen::Vector3d &velocity) {
    vel_ = velocity;
}
...
bool
DDControl::checkReachedGoal(const Eigen::Vector3d &des_pos, double des_yaw) {
    bool result = false;
    double dx = pos_[0] - des_pos[0];
    double dy = pos_[1] - des_pos[1];
    double dyaw = yaw_ - des_yaw;
    if (std::abs(dyaw) < M_PI / 6 && std::hypot(dx, dy) < 0.3) result = true;
    return result;
}

void
DDControl::setHeading(double heading) {
    yaw_ = heading;
}

double
DDControl::getLinearVelocity() const {
    return v;
}

double
DDControl::getAngularVelocity() const {
    return w;
}
