...
public:
    ...
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;  // Need this since we have SO3Control which needs aligned
                                      // pointer
...
private:
    ...
    Eigen::Vector3d des_pos_, des_vel_, des_acc_, des_jrk_, kx_, kv_, ki_;
...
void
DDControlNode::publishDDCommand(void) {
    if (!have_odom_) {
        ROS_WARN("No odometry! Not publishing DDCommand.");
        return;
    }

    controller_.calculateControl(des_pos_[0], des_pos_[1], des_yaw_, kp, ka, kb, vmax, wmax);
...
void
DDControlNode::publishZeroCommand(void) {
    if (!have_odom_) {
...
void
DDControlNode::position_cmd_callback(const geometry_msgs::PoseStamped::ConstPtr &cmd) {
    ...
    ROS_INFO_THROTTLE(
        2.0,
        "Received position command %f %f %f\n",
        des_pos_[0],
        des_pos_[1],
        des_yaw_);
    publishDDCommand();
...
void
DDControlNode::odom_callback(const nav_msgs::Odometry::ConstPtr &odom) {
    have_odom_ = true;
...
DDControlNode::onInit(void) {
    ...
    odom_sub_ = n.subscribe(
        "odom",
        10,
        &DDControlNode::odom_callback,
        this,
        ros::TransportHints().tcpNoDelay());
    position_cmd_sub_ = n.subscribe(
        "position_cmd",
        10,
        &DDControlNode::position_cmd_callback,
        this,
        ros::TransportHints().tcpNoDelay());
    // Setup Publisher
    ...
    n.param("gains/kp", kp, 3.0);
    n.param("gains/ka", ka, 8.0);
    n.param("gains/kb", kb, -1.5);
...
main(int argc, char **argv) {
    ros::init(argc, argv, "dd_control_node");
    DDControlNode node;
